...
# NOTES
# Added by todo:
# Adds support for newer version of chinese BMS, based on the 'Daren'/SNS01 Ho1/Ho2 BMS types
# e.g. SNS01 Ho20-T1 DR-1363 protocol.
# See https://github.com/cpttinkering/daren-485 for original protocol research information but keep in mind
# the new version has moved things around a bit and started using a new SOI
...
class Daren485v2(Battery):
    def __init__(self, port, baud, address):
        super(Daren485v2, self).__init__(port, baud, address)
    ...
    def probe(self, ser, cid1=b"\x4A", cid2=b"\x84", raw=None):
        result = False

        if not raw:
            #  req = self.create_command(self.address, cid1, cid2, self.address.hex().upper())
            req = self.create_command(self.address, cid1, cid2)
        if raw:
            req = raw
        ...
        sleep(4.5)  # Allow the BMS some time to send a full response
        ...
        if response:
            ...
            logger.info(f"reponse length: {len(response)}")
        else:
            logger.debug("No response to this probe.")
    ...
    def get_serial(self, ser):
        ...
        if response:
            ...
            if len(payload) >= 30:
                serial_byte_array = bytearray.fromhex(payload[:30])
                self.serial_number = serial_byte_array.decode("ascii", errors="ignore")
    ...
    def create_command_get_cells_params(self):
        ...
        ~22014A47E00201FD23␍
        """
        return self.create_command(self.address, b"\x42", b"\x47", self.address.hex().upper())
    ...
    def create_command_get_realtime_data(self):
        ...
        return self.create_command(self.address, b"\x42", b"\x42")
    ...
    def create_command_get_manufacturer_info(self):
        ...
        ~22014A510000FDA0␍
        """
        return self.create_command(self.address, b"\x42", b"\x51")
